...
//    thickness/2  / x  = tan(2*PI/24)
// Solving
//   radius^2 = thickness^2 (  1/4   +  (1+1/(2*tan(2*PI/24)))^2  )

//---------------------------------------------------------------------------------
var LedWheel = React.createClass({
  render: function() {
    var radius = this.props.radius == undefined ? 200 : parseInt(this.props.radius);
    var thickness = radius / Math.sqrt(0.25 + (1 + 1/(2*Math.tan(2*Math.PI/24)))^2 );
    var r1 = thickness / (2 * Math.tan(2*Math.PI/24));
    var r2 = thickness / (2 * Math.tan(2*Math.PI/12));
